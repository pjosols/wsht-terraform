...
| <a name="input_routes"></a> [routes](#input\_routes) | List of routes to create. Each route has method, path, authorizer (NONE or a key from authorizer\_configs), optional required scopes, and optional per-route throttle overrides. | <pre>list(object({<br/>    method                 = string<br/>    path                   = string<br/>    authorizer             = optional(string, "NONE")<br/>    scopes                 = optional(list(string), [])<br/>    throttling_rate_limit  = optional(number)<br/>    throttling_burst_limit = optional(number)<br/>  }))</pre> | n/a | yes |
